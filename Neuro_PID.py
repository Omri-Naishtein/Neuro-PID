...
GYRO_DEADBAND  = 1.0
# ── MLP WEIGHTS PATH ─────────────────────────────────────────────────────────
MLP_WEIGHTS    = "/home/ubuntu/mlp_weights.bin"
# ── HELPERS ───────────────────────────────────────────────────────────────────
...
class Robot(Node):
    def __init__(self):
        ...
        # ── PID controllers (NeuroPID) ────────────────────────────────────
        self.pid_head = PID(kp=2.0, ki=0.15, kd=0.05,
                            out_min=-20, out_max=20, frozen=True)

        self.pid_dist = PID(kp=70.0, ki=0.5, kd=8.0,
                            out_min=0, out_max=CRUISE_PWM,
                            gain_limits={"kp": (10, 120),
                                         "ki": (0, 5),
                                         "kd": (0, 20)})
        self.pid_dist.load(MLP_WEIGHTS)

        self.pid_turn = PID(kp=2.0, ki=0.3, kd=0.7,
                            out_min=-PWM_MAX, out_max=PWM_MAX, frozen=True)
        # ──────────────────────────────────────────────────────────────────
        self._stop_motors()
    ...
    def control(self):
        ...
        if self.state == "FORWARD":
            ...
            kp, ki, kd = self.pid_dist.gains()
            print(f"[NEURO-PID] dist={self.dist_front*100:.0f}cm  speed={speed:.0f}%  "
                  f"Kp={kp:.1f} Ki={ki:.2f} "
                  f"Kd={kd:.1f}  L={left:.0f}%  R={right:.0f}%")
    ...
    def stop(self):
        ...
        self.left_pwm.stop()
        self.right_pwm.stop()
        self.pid_dist.save(MLP_WEIGHTS)
        print(f"MLP weights saved ({self.pid_dist.step_count} learning steps)")
# ── MAIN ──────────────────────────────────────────────────────────────────────
def main():
    rclpy.init()
    print("Waiting 15 seconds before starting...")
    time.sleep(15)
    print("Starting robot initialization and calibration...")
    bot = Robot()
    try:
        while rclpy.ok():
